...
import { Loader2, Search, ArrowRight, AlertCircle, CheckCircle, Info } from 'lucide-react';
...
export default function WebScraper() {
  ...
  const validateUrl = (inputUrl: string) => {
    ...
    const selectedPlatformRegex = platformUrls[platform as keyof typeof platformUrls];
    
    if (!selectedPlatformRegex.test(inputUrl)) {
      setUrlError(`Please enter a valid ${platform} URL. Example: https://www.${platform}.com/property`);
      return false;
    } else {
      setUrlError('');
      return true;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isFallback) {
        setScraperInfo('The scraper service is currently unavailable. Using fallback data for demonstration purposes.');
      } else if (useMockData) {
        setScraperInfo('Using demo data as requested. For real analysis, uncheck "Use demo data" and enter a valid property URL.');
      } else {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-lg shadow-lg p-6">
      ...
      <p className="mb-6 text-slate-600 dark:text-slate-300">
        Enter a property listing URL from a Spanish real estate website to analyze it using our AI model.
      </p>
        ...
        <>
          ...
          <div className="rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="pt-4 mt-4 border-t border-slate-200 dark:border-slate-700 text-xs text-slate-500 dark:text-slate-400">
                      ...
                      {propertyData.source.scrapedAt && (
                        <div className="flex justify-between">
                          ...
                          <span>{new Date(propertyData.source.scrapedAt).toLocaleString()}</span>
                        </div>
                      )}
                    </div>
                  )}
              ...
              <div>
                <h4 className="font-semibold mb-3 text-slate-900 dark:text-white">Risk Assessment</h4>
                <div className="mb-4">
                  <div className="flex justify-between mb-1">
                    <span className="text-slate-600 dark:text-slate-400">Atlas AI Score:</span>
                    <span className="font-medium text-slate-900 dark:text-white">
                  ...
                  <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-2.5">
                    <div 
                      className={`h-2.5 rounded-full ${getScoreColorClass(propertyData.atlasScore)}`}
            ...
            {propertyData.aiAnalysis && (
              <div className="p-4 bg-blue-50 dark:bg-blue-900/10 border-t border-slate-200 dark:border-slate-700">
                <h4 className="font-semibold mb-2 text-slate-900 dark:text-white">AI Analysis</h4>
                <p className="text-sm text-slate-600 dark:text-slate-300">{propertyData.aiAnalysis}</p>
              </div>
            )}
